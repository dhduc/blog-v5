...
<div {{ $attributes }}>
    <a href="{{ $link->url }}" target="_blank" class="relative">
        @if ($link->image_url)
        ...
        @else
            @php
            $bgColors = collect([
                'bg-amber-600', 'bg-primary-600', 'bg-cyan-600', 'bg-emerald-600', 'bg-gray-600', 'bg-green-600', 'bg-indigo-600', 'bg-lime-600', 'bg-pink-600', 'bg-purple-600', 'bg-red-600', 'bg-sky-600', 'bg-teal-600', 'bg-yellow-600',
            ])->random();
            @endphp
    ...
    <div class="flex items-center justify-between gap-6 mt-2">
        <a href="{{ $link->url }}" target="_blank" class="font-bold transition-colors text-xl/tight hover:text-primary-600">
            {{ $link->title }}
        </a>
    </div>
